gboom_sampler_mod <- function(y,
  ...
  sam_l2_r2_t2_s2B <- array(data = NA, dim = c(S, V, P))
  sam_l2_r2_t2B    <- array(data = NA, dim = c(S, V, P))
  sam_l2_r2B       <- array(data = NA, dim = c(S, V, P))
  sam_l2B          <- array(data = NA, dim = c(S, V, P))
  sam_r2B          <- matrix(data = NA, nrow = S, ncol = P)
  sam_t2B          <- numeric(length = S)
  sam_vB           <- array(data = NA, dim = c(S, V, P))
  sam_wB           <- matrix(data = NA, nrow = S, ncol = P)
  ...
  for(s in 1:S){
    ...
    sam_Theta[s,,]         <- Theta
    sam_s2[s]              <- s2
    sam_l2_r2_t2_s2B[s,,]  <- t(t(l2B) * r2B) * t2B * s2
    sam_l2_r2_t2B[s,,]     <- t(t(l2B) * r2B) * t2B
    sam_l2_r2B[s,,]        <- t(t(l2B) * r2B)
    sam_l2B[s,,]           <- l2B
    sam_r2B[s,]            <- r2B
    sam_t2B[s]             <- t2B
    sam_vB[s,,]            <- vB
